...
const SHELL_FILES = [
  ...
  "./icon-192.png",
  "./icon-512.png",
  /* Η εφαρμογή προπόνησης. Ίδιο origin, ίδιος service worker: ο
     διακόπτης στην κεφαλίδα δεν βγάζει τον πελάτη από την εφαρμογή
     και όλα δουλεύουν και χωρίς σήμα. */
  "./workout/index.html",
  "./workout/app.js",
  "./workout/data.js",
  "./workout/storage.js",
...
const isWorkoutPage = (url) => /\/workout\/(index\.html)?$/.test(url.pathname);
const isClientPage = (url) =>
  !isWorkoutPage(url) && /\/(index\.html)?$/.test(url.pathname);
...
async function networkFirst(request) {
  ...
  try {
    ...
    /* Η εφεδρεία είναι η εφαρμογή πελατών μόνο όταν ζητήθηκε αυτή.
       ...
       ως εφαρμογή πελατών. */
    if (isWorkoutPage(url)) {
      const shell = await caches.match("./workout/index.html");
